...
configure :notify do |s| # {{{
  ...
  s.win.on :draw do
    ...
    s.dbus.messages.each_index do |i|
      ...
      s.win.draw_text(2, y,
        s.dbus.messages[i].summary[0..50])
    end
...
on :mouse_over do |s| # {{{
  ...
  unless s.dbus.messages.empty?
    widest = s.dbus.messages.sort { |a, b|
      a.summary.size <=> b.summary.size
    }.last

    x      = 0
    y      = 0
    width  = s.win.font_width(widest.summary[0..50]) + 4
    height = s.dbus.messages.size * s.win.font_height + 4
