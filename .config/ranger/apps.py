    ...
    def app_default(self, c):
        ...
        if f.extension is not None:
            if f.extension in ('pdf', ):
                return self.either(c, 'evince', 'zathura', 'mupdf', 'llpp', 
                                   'apvlv', 'okular', 'epdfview')
            if f.extension == 'djvu':
            ...
            if f.extension == 'doc':
                return self.either(c, 'abiword', 'libreoffice', 'soffice', 'ooffice')
            if f.extension == 'chm':
                c.flags += 'd'
        ...
        if f.video or f.audio:
            if f.video:
                c.flags += 'd'
                return self.either(c, 'mplayer2', 'mplayer', 'gmplayer',
                                   'smplayer', 'vlc', 'totem')
