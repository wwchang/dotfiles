...
# Time-stamp: <subi 2011/03/02 18:18:45>
#
...
EOF
    ...
    if [[ ${#FILES[@]} == "1" ]]; then
        ...
        ### FIXME: consider using keywords like "EP", "CD", "Ã§Â¬Â¬ Ã©âºâ ", etc.?
...
configure_local () {
    ...
    if is_stream "$@"; then
        ...
        DIRNAME=$(basename $(dirname "$CANONICAL_NAME"))
...
gen_query () {
    local AWKSCRIPT
    # Ã¦â Ã¨â¬Â»Ã¦Â·Â·Ã¨âºâ¹.Inglourious.Basterds.2009.BDRip.X264-TLF(548063E9).mkv -> BDRip+Inglourious+Basterds+TLF
...
fetch_subtitle () {
    ...
    ## load subtitles
    load_subs () {
        # XXX I added one ssa subtitle format
        set -- $(find "${SUBTITLES_DIR}/${FILEID}" -iregex '.*[srt|ass|ssa]$')
        [[ "$#" -eq 0 ]] &&return 1
