# is a modified version of cras' script. Dictcomplete lets you tab-complete words from the /usr/share/dict/words file. Word lists are available for many languages.
# Instead of parsing the entire word list on each completion, this modified version reads the list into memory. It's a lot faster than the original, but it also uses
# more resources.
use strict;
